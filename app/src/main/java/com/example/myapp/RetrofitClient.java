...
public class RetrofitClient {
    ...
    public static ApiService getApiService() {
        if (retrofit == null) {
            // 4. Thiết lập Interceptor và Authenticator
            OkHttpClient client = new OkHttpClient.Builder()
                    ...
                    // Tự động Refresh khi nhận lỗi 401 (Hết hạn Access Token)
                    .authenticator((route, response) -> {
                        String refreshToken = SharedPrefsManager.getRefreshToken();
                        if (refreshToken == null) return null;

                        // Gọi API Refresh đồng bộ (.execute) để lấy token mới
                        ...
                        return null; // Nếu refresh thất bại, buộc user login lại
                    })
                    .build();
